...
class DiscoverViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, UITextFieldDelegate {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if segue.identifier == "discoverViewSegue" {
            let selectedIndexPath = discoverTable.indexPath(for: sender as! DiscoverTableViewCell)!
            let destinationController = segue.destination as! LocationDetailedViewController
            var carry = discover[selectedIndexPath.row]
            if filtered {
                carry = filteredDiscover[selectedIndexPath.row]
            }
